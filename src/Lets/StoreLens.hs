...
product ::
  ...
  -> Lens c d
  -> Lens (a, c) (b, d)
product f g = Lens $ \(a, c) -> Store (\(b, d) -> (set f a b, set g c d)) (get f a, get g c)
...
choice ::
  ...
  -> Lens b x
  -> Lens (Either a b) x
choice l r = Lens $ either leftStore rightStore
  where
    leftStore a  = Store (Left . set l a) (get l a)
    rightStore b = Store (Right . set r b) (get r b)
